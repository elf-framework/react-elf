...
export const DefaultToast: Story = (args) => {
  const [{ content, closable, direction, position, variant }, updateArgs] = useArgs();
  ...
  return <div >
    <Toast variant={variant} closable={closable} direction={direction} style={{
      position
    }} onClose={handleClose}>{content}</Toast>
  </div >;
}